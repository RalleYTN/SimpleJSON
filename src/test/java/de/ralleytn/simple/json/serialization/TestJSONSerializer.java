...
public class TestJSONSerializer {
	...
	public void testDeserializeVarious() {
		...
		assertDoesNotThrow(() -> JSONSerializer.deserialize(new JSONObject(EXPECTED_VARIOUS), container));
		assertEquals(container, new TestDataContainerVarious());
	}
	
	@Test
	public void testDeserializePrimitiveArrays() {
		
		TestDataContainerPrimitiveArrays container = new TestDataContainerPrimitiveArrays().empty();
		assertDoesNotThrow(() -> JSONSerializer.deserialize(new JSONObject(EXPECTED_PRIMITIVE_ARRAYS), container));
		assertEquals(container, new TestDataContainerPrimitiveArrays());
	}
	
	@Test
	public void testDeserializeSimple() {
		
		TestPersonWithGettersAndSetterMethods person1 = new TestPersonWithGettersAndSetterMethods();
		assertDoesNotThrow(() -> JSONSerializer.deserialize(new JSONObject(EXPECTED_SIMPLE), person1));
		testJohnDoe(person1);
		assertNull(person1.getFriends());
		
		TestPersonWithPublicFields person2 = new TestPersonWithPublicFields();
		assertDoesNotThrow(() -> JSONSerializer.deserialize(new JSONObject(EXPECTED_SIMPLE), person2));
		testJohnDoe(person2);
		assertNull(person2.friends);
	}
	...
	public void testSerializePrimitiveArrays() {
		...
		assertDoesNotThrow(() -> assertEquals(EXPECTED_PRIMITIVE_ARRAYS, JSONSerializer.serialize(new TestDataContainerPrimitiveArrays()).toString()));
	}
	
	@Test
	public void testSerializeSimple() {
		
		assertDoesNotThrow(() -> assertEquals(EXPECTED_SIMPLE, JSONSerializer.serialize(createJohnDoe()).toString()));
		assertDoesNotThrow(() -> assertEquals(EXPECTED_SIMPLE, JSONSerializer.serialize(createJohnDoeBean()).toString()));
	}
	...
	private static final TestPersonWithPublicFields createJohnDoe() {
		...
		TestPersonWithPublicFields person = new TestPersonWithPublicFields();
		person.firstName = "John";
		person.lastName = "Doe";
		person.age = 21;
		person.birthDate = LocalDate.of(1998, 6, 1);
		person.city = "Example City";
		person.country = "Example Country";
		person.friends = null;
		person.bloodDonor = true;
		person.gender = "Manly Male Masculine Man";
	...
	private static final TestPersonWithGettersAndSetterMethods createJohnDoeBean() {
		...
		TestPersonWithGettersAndSetterMethods person = new TestPersonWithGettersAndSetterMethods();
		person.setFirstName("John");
		person.setLastName("Doe");
		person.setAge(21);
		person.setBirthDate(LocalDate.of(1998, 6, 1));
		person.setCity("Example City");
		person.setCountry("Example Country");
		person.setFriends(null);
		person.setBloodDonor(true);
		person.setGender("Manly Male Masculine Man");
		person.setPostalCode("12345");

		return person;
	}
	
	private static final void testJohnDoe(TestPersonWithGettersAndSetterMethods person) {
